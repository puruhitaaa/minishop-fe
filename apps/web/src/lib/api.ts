...
export async function apiRequest<T>(path: string, options: ApiRequestOptions = {}): Promise<T> {
  const { method = "GET", query, json, form, admin, okStatuses = [200, 201] } = options;
...
export async function listPublicProducts(
  query?: NonNullable<import("../../../../src/api/types").operations["listPublicProducts"]["parameters"]["query"]>,
) {
...
export async function listAdminCategories(
  query?: NonNullable<import("../../../../src/api/types").operations["listAdminCategories"]["parameters"]["query"]>,
) {
  return apiRequest<CategoryListData>("/internal/categories", { query, admin: true });
}

export async function listAdminProducts(
  query?: NonNullable<import("../../../../src/api/types").operations["listAdminProducts"]["parameters"]["query"]>,
) {
  return apiRequest<ProductListData>("/internal/products", { query, admin: true });
}

export async function getAdminProduct(idOrSlug: string) {
  return apiRequest<Product>(`/internal/products/${encodeURIComponent(idOrSlug)}`, { admin: true });
}
...
export async function updateAdminProduct(idOrSlug: string, form: FormData) {
  return apiRequest<Product>(`/internal/products/${encodeURIComponent(idOrSlug)}`, {
    method: "PATCH",
    form,
    admin: true,
  });
...
export async function deleteAdminProduct(idOrSlug: string) {
  return apiRequest<null>(`/internal/products/${encodeURIComponent(idOrSlug)}`, {
    method: "DELETE",
    admin: true,
  });
}
...
export async function listAdminOrders(
  query?: NonNullable<import("../../../../src/api/types").operations["listAdminOrders"]["parameters"]["query"]>,
) {
...
export async function getAdminOrder(idOrOrderNumber: string) {
  return apiRequest<Order>(`/internal/orders/${encodeURIComponent(idOrOrderNumber)}`, { admin: true });
}